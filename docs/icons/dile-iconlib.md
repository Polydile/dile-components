...
The `dile-iconlib` Custom Element provides a single, flexible way to display an icon from **any** of the supported icon libraries — Lucide, Material, FontAwesome, Phosphor or Tabler — using one tag and a compound `icon` attribute, following the Web Components standard.

The `icon` attribute value follows the `<family>.<name>` format, where `family` is one of `lucide`, `material`, `fontawesome`, `phosphor` or `tabler`:
...
- **icon**: String combining the family and the icon name as `family.name`, e.g. `"lucide.house"`, `"material.receipt"`, `"fontawesome.microsoft"`, `"phosphor.house-bold"`, `"tabler.heart-filled"`. Supported families: `lucide`, `material`, `fontawesome`, `phosphor`, `tabler`. For Phosphor, the weight (`bold`, `duotone`, `fill`, `light`, `thin`) is part of the name suffix; a bare name like `phosphor.house` uses the regular weight. For Tabler, the `-filled` suffix selects the filled variant; a bare name like `tabler.heart` uses the outline variant. If the family isn't recognized, a warning is logged to the console and nothing is rendered; if `icon` is omitted, a default icon (`lucide.dot`) is shown.
- **rounded**: Boolean attribute for a button-like, circular background.

> To find the specific name of each icon, search the [Icon Catalog](../icon-catalog/), or check the [Lucide icons](../lucide-icons/), [Material icons](../material-icons/), [FontAwesome icons](../fontawesome-icons/), [Phosphor icons](../phosphor-icons/) or [Tabler icons](../tabler-icons/) web components pages.

## Styling and Features

This component inherits all styling options and CSS custom properties from the underlying family-specific icon components documented in [Lucide Icons](../lucide-icons/), [Material Icons](../material-icons/), [FontAwesome Icons](../fontawesome-icons/), [Phosphor Icons](../phosphor-icons/) and [Tabler Icons](../tabler-icons/) Web Components.
...
`dile-iconlib` is an additional, convenience component. The original per-family components (`dile-lucide-icon`, `dile-material-icon`, `dile-fontawesome-icon`, `dile-phosphor-icon`, `dile-tabler-icon`) remain available and unchanged — see [Lucide Generic](../lucide-icon/), [Material Generic](../material-icon/), [FontAwesome Generic](../fontawesome-icon/), [Phosphor Generic](../phosphor-icon/) and [Tabler Generic](../tabler-icon/).
