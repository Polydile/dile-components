...
const BRAND_ICONS_DIR = path.join(__dirname, '..', 'src-icons/font-awesome/regular');
const COMPONENTS_OUTPUT_DIR = path.join(__dirname, '..', 'packages', 'iconlib', 'fontawesome-icons');
...
function svgFileNameToCustomElementName(fileName) {
  const nameWithoutExtension = fileName.replace('.svg', '');
  return `dile-fontawesome-icon-regular-${nameWithoutExtension}`;
}
...
function generateComponents() {
  ...
  svgFiles.forEach((svgFile, index) => {
    try {
      ...
      const componentFilePath = path.join(COMPONENTS_OUTPUT_DIR, 'regular-' + componentFileName);
