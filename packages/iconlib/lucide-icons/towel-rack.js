import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibTowelRack extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M22 7h-2" /> <path d="M6.5 3h11A2.5 2.5 0 0 1 20 5.5V20a1 1 0 0 1-1 1h-9a1 1 0 0 1-1-1V5.5a1 1 0 0 0-5 0V17a1 1 0 0 0 1 1h4" /> <path d="M9 7H2" /></svg>`;
  }
}

if (!customElements.get('dile-lucide-icon-towel-rack')) {
  customElements.define('dile-lucide-icon-towel-rack', DileIconlibTowelRack);
}
