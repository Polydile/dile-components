import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibPanelLeftRightDashed extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M15 10V9" /> <path d="M15 15v-1" /> <path d="M15 21v-2" /> <path d="M15 5V3" /> <path d="M9 10V9" /> <path d="M9 15v-1" /> <path d="M9 21v-2" /> <path d="M9 5V3" /> <rect x="3" y="3" width="18" height="18" rx="2" /></svg>`;
  }
}

if (!customElements.get('dile-lucide-icon-panel-left-right-dashed')) {
  customElements.define('dile-lucide-icon-panel-left-right-dashed', DileIconlibPanelLeftRightDashed);
}
