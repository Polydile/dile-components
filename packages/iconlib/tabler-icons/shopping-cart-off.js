import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibShoppingCartOff extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M4 19a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M17 17a2 2 0 1 0 2 2" /> <path d="M17 17h-11v-11" /> <path d="M9.239 5.231l10.761 .769l-1 7h-2m-4 0h-7" /> <path d="M3 3l18 18" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-shopping-cart-off')) {
  customElements.define('dile-tabler-icon-shopping-cart-off', DileIconlibShoppingCartOff);
}
