import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibBrowserMaximize extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M4 8h8" /> <path d="M20 11.5v6.5a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2h6.5" /> <path d="M8 4v4" /> <path d="M16 8l5 -5" /> <path d="M21 7.5v-4.5h-4.5" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-browser-maximize')) {
  customElements.define('dile-tabler-icon-browser-maximize', DileIconlibBrowserMaximize);
}
