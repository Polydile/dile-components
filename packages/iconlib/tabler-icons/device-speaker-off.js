import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibDeviceSpeakerOff extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M7 3h10a2 2 0 0 1 2 2v10m0 4a2 2 0 0 1 -2 2h-10a2 2 0 0 1 -2 -2v-14" /> <path d="M11.114 11.133a3 3 0 1 0 3.754 3.751" /> <path d="M12 7v.01" /> <path d="M3 3l18 18" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-device-speaker-off')) {
  customElements.define('dile-tabler-icon-device-speaker-off', DileIconlibDeviceSpeakerOff);
}
