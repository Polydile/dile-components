import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibCurrencyReal extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M21 6h-4a3 3 0 0 0 0 6h1a3 3 0 0 1 0 6h-4" /> <path d="M4 18v-12h3a3 3 0 1 1 0 6h-3c5.5 0 5 4 6 6" /> <path d="M18 6v-2" /> <path d="M17 20v-2" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-currency-real')) {
  customElements.define('dile-tabler-icon-currency-real', DileIconlibCurrencyReal);
}
