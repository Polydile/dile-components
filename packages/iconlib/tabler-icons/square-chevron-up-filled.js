import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibSquareChevronUpFilled extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"> <path d="M19 2a3 3 0 0 1 3 3v14a3 3 0 0 1 -3 3h-14a3 3 0 0 1 -3 -3v-14a3 3 0 0 1 3 -3zm-6.387 7.21a1 1 0 0 0 -1.32 .083l-3 3l-.083 .094a1 1 0 0 0 .083 1.32l.094 .083a1 1 0 0 0 1.32 -.083l2.293 -2.292l2.293 2.292l.094 .083a1 1 0 0 0 1.32 -1.497l-3 -3z" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-square-chevron-up-filled')) {
  customElements.define('dile-tabler-icon-square-chevron-up-filled', DileIconlibSquareChevronUpFilled);
}
