import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibBuildingCastle extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M15 19v-2a3 3 0 0 0 -6 0v2a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1v-14h4v3h3v-3h4v3h3v-3h4v14a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1" /> <path d="M3 11l18 0" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-building-castle')) {
  customElements.define('dile-tabler-icon-building-castle', DileIconlibBuildingCastle);
}
