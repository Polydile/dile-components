import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibFileScissors extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M14 3v4a1 1 0 0 0 1 1h4" /> <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2" /> <path d="M14 17a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" /> <path d="M8 17a1 1 0 1 0 2 0a1 1 0 1 0 -2 0" /> <path d="M9 17l6 -6" /> <path d="M15 17l-6 -6" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-file-scissors')) {
  customElements.define('dile-tabler-icon-file-scissors', DileIconlibFileScissors);
}
