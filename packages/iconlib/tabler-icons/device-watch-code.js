import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibDeviceWatchCode extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M11 18h-2a3 3 0 0 1 -3 -3v-6a3 3 0 0 1 3 -3h6a3 3 0 0 1 3 3v4" /> <path d="M20 21l2 -2l-2 -2" /> <path d="M17 17l-2 2l2 2" /> <path d="M9 18v3h3" /> <path d="M9 6v-3h6v3" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-device-watch-code')) {
  customElements.define('dile-tabler-icon-device-watch-code', DileIconlibDeviceWatchCode);
}
