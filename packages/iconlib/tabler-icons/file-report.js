import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibFileReport extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M13 17a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /> <path d="M17 13v4h4" /> <path d="M12 3v4a1 1 0 0 0 1 1h4" /> <path d="M11.5 21h-6.5a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v2m0 3v4" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-file-report')) {
  customElements.define('dile-tabler-icon-file-report', DileIconlibFileReport);
}
