import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibNavigationSouth extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M10 8.25c0 .414 .336 .75 .75 .75h2.25a1 1 0 0 0 1 -1v-1a1 1 0 0 0 -1 -1h-2a1 1 0 0 1 -1 -1v-1a1 1 0 0 1 1 -1h2.25a.75 .75 0 0 1 .75 .75" /> <path d="M16 21l-4 -8l-4 8l4 -2l4 2" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-navigation-south')) {
  customElements.define('dile-tabler-icon-navigation-south', DileIconlibNavigationSouth);
}
