import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibLassoPolygon extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M4.028 13.252l-1.028 -3.252l2 -7l7 5l8 -3l1 9l-9 3l-5.144 -1.255" /> <path d="M3 15a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /> <path d="M5 17c0 1.42 .316 2.805 1 4" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-lasso-polygon')) {
  customElements.define('dile-tabler-icon-lasso-polygon', DileIconlibLassoPolygon);
}
