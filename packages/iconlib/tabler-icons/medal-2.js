import { DileBaseIcon } from '../src/DileBaseIcon.js';

export class DileIconlibMedal2 extends DileBaseIcon {
  getSvgIcon() {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M9 3h6l3 7l-6 2l-6 -2l3 -7" /> <path d="M12 12l-3 -9" /> <path d="M15 11l-3 -8" /> <path d="M12 19.5l-3 1.5l.5 -3.5l-2 -2l3 -.5l1.5 -3l1.5 3l3 .5l-2 2l.5 3.5l-3 -1.5" /></svg>`;
  }
}

if (!customElements.get('dile-tabler-icon-medal-2')) {
  customElements.define('dile-tabler-icon-medal-2', DileIconlibMedal2);
}
